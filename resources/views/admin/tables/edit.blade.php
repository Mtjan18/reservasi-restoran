...
<head>
    ...
    <title>Admin - Edit Table {{ $table->tableNumber }}</title>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            <header class="bg-white shadow-md">
                <div class="flex items-center justify-between px-6 py-4">
                    <h1 class="text-3xl font-semibold text-gray-800">Edit Table: {{ $table->tableNumber }}</h1>
                </div>
            </header>
                    ...
                    <form action="{{ route('admin.tables.update', $table->tableId) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="number" id="tableNumber" name="tableNumber" value="{{ old('tableNumber', $table->tableNumber) }}" required 
                            ...
                            <div>
                                ...
                                <select id="status" name="status" required
                                    ...
                                    <option value="reserved" {{ old('status', $table->status) == 'reserved' ? 'selected' : '' }}>Reserved (Temporarily unavailable)</option>
                                    <option value="unavailable" {{ old('status', $table->status) == 'unavailable' ? 'selected' : '' }}>Unavailable (Under maintenance)</option>
                                </select>
                                @error('status')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
